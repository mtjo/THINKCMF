package com.base.aframe.ui.widget.xlistview;
 ...
 * 下拉加载更多，使用于对话框的分页
 */
public class XChatListView extends ListView implements OnScrollListener{
	 private float mLastY = -1; // save event y
	    private Scroller mScroller; // used for scroll back
	    private OnScrollListener mScrollListener; // user's scroll listener

	    // the interface to trigger refresh and load more.
	    private DataQueryInerface mListViewListener;
	    ...
	    private int queryType = 0; //查询的类型：0：初始化加载，1：刷新加载，2：加载更多
	    ...
	    public XChatListView(Context context, AttributeSet attrs, int defStyle) {
...
//	        mFooterView.setState(XFooterView.STATE_LOADING);
	        if (mListViewListener != null && !mPullLoading && !mPullRefreshing) {
	        	...
	        	mPullLoading = true;
	        	queryType =2;
	            mListViewListener.onLoadMore(pageNo+1,pageSize);
	        }
	    }
...
//	                        mHeaderView.setState(XHeaderView.STATE_REFRESHING);
	                        if (mListViewListener != null && !mPullRefreshing && !mPullLoading) {
	                        	mPullRefreshing = true;
	                        	queryType = 1;
	                            mListViewListener.onRefresh(pageNo+1,pageSize);
	                        }
	                    }
	                    resetHeaderHeight();
	    ...
	    public void setXListViewListener(DataQueryInerface l) {
	        mListViewListener = l;
	    }
	     ...
	     * 加载完毕
	     */
	    public void loadOk(boolean success){
	    	stopRefresh();
	    	if(success && queryType == 1)pageNo++;
	    }
