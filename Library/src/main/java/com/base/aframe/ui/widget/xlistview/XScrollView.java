...
//    private static final String TAG = "XScrollView";
    ...
    private DataQueryInerface mScorllViewListener;
...
//        mFooterView.setState(XFooterView.STATE_LOADING);
        if (mScorllViewListener != null && !mPullLoading && !mPullRefreshing) {
        	mPullLoading = true;
        	mFooterView.setState(XFooterView.STATE_LOADING);
        	queryType = 2;
            mScorllViewListener.onLoadMore(pageNo+1,pageSize);
        }
    }
...
//                mHeaderView.setState(XHeaderView.STATE_REFRESHING);
                if (mScorllViewListener != null && !mPullRefreshing && !mPullLoading) {
                	mHeaderView.setState(XHeaderView.STATE_REFRESHING);
                	mPullRefreshing = true;
                	queryType = 1;
                    mScorllViewListener.onRefresh(1,pageSize);
                }
            }
            resetHeaderHeight();
...
//        		(getScrollY() >= 0 && null != mFooterView && mFooterView.getBottomMargin() >= 0);
     ...
     * @param l
     */
    public void setIXScrollViewListener(DataQueryInerface l) {
