 ...
 */
package com.base.aframe.database;
...
import com.base.aframe.database.utils.KeyValue;
import com.base.aframe.database.utils.OneToMany;
import com.base.aframe.database.utils.TableInfo;
import com.base.aframe.core.WYException;
import com.base.aframe.database.utils.ManyToOne;
...
public class WYDB {
    ...
    /** 用于保存所有数据库 */
    private static HashMap<String, WYDB> daoMap = new HashMap<String, WYDB>();

    private SQLiteDatabase db;
    private DaoConfig config;

    private WYDB(DaoConfig config) {
        if (config == null) {
            throw new WYException("daoConfig is null");
        }
        if (config.getContext() == null) {
            throw new WYException("android context is null");
        }
    ...
    private synchronized static WYDB getInstance(DaoConfig daoConfig) {
        WYDB dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new WYDB(daoConfig);
     ...
     * @param context
     */
    public static WYDB create(Context context) {
     ...
     */
    public static WYDB create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     *            数据库名称
     */
    public static WYDB create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static WYDB create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     *            数据库名称
     */
    public static WYDB create(Context context, String targetDirectory,
            String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static WYDB create(DaoConfig daoConfig) {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                try {
                	...
                	Loger.debug(getClass().getName() + e.getMessage());
                }
            }
        }
        if (cursor != null) {
    ...
    private void exeSqlInfo(SqlInfo sqlInfo) {
        if (sqlInfo != null) {
        	...
        	Loger.debug(getClass().getName() + "sava error:sqlInfo is null");
        }
    }
     ...
     *            条件为空的时候查找所有数据
     */
    public <T> List<T> findBySql(Class<T> clazz, String strSQL) {
    	checkTableExist(clazz);
     ...
     * 计算总和
     */
    public <T> int sumSql(Class<T> clazz, String strSQL, String sum){
    	checkTableExist(clazz);
    	int count = 0;
    	Cursor cursor = db.rawQuery(strSQL, null);
        try {
    ...
    private SQLiteDatabase createDbFileOnSDCard(String sdcardPath,
        ...
        if (!dbf.exists()) {
            try {
                ...
                throw new WYException("数据库文件创建失败", ioex);
            }
        } else {
            return SQLiteDatabase.openOrCreateDatabase(dbf, null);
        }
